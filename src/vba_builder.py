...
VBA_SOURCE = (
    ...
    "        count = count + 1\r\n"
    "    Next i\r\n"
    "    MsgBox count & \" facture(s) ajoutee(s).\" & vbCrLf & _\r\n"
    "           \"Elles seront incluses dans le prochain PDF.\", _\r\n"
    "           vbInformation, \"Factures ajoutees\"\r\n"
    "End Sub\r\n"
    "\r\n"
    "' ----------------------------------------------------------------\r\n"
    "'  Bouton 2 : Generer le PDF final\r\n"
    "' ----------------------------------------------------------------\r\n"
    "Sub GenererPDF()\r\n"
    "    ThisWorkbook.Save\r\n"
    "\r\n"
    ...
    "    outFolder = base & \"output\" & Application.PathSeparator\r\n"
    "    EnsureFolder outFolder\r\n"
    "\r\n"
    "    Dim q As String : q = Chr(34)\r\n"
    "    Dim formPdf As String  : formPdf  = outFolder & \"formulaire.pdf\"\r\n"
    "    Dim finalPdf As String : finalPdf = outFolder & \"final.pdf\"\r\n"
    "    Dim logPath As String  : logPath  = outFolder & \"log.txt\"\r\n"
    "\r\n"
    "    ' --- Etape 1 : export du formulaire en PDF via Excel ---\r\n"
    ...
    "\r\n"
    "ErrExport:\r\n"
    "    MsgBox \"Erreur export PDF : \" & Err.Description, vbCritical, \"Erreur\"\r\n"
    "    On Error GoTo 0\r\n"
    "    Exit Sub\r\n"
    "\r\n"
    "AfterExport:\r\n"
    ...
    "\r\n"
    "    ' --- Etape 2 : appel Python pour fusion avec les factures ---\r\n"
    "    Dim scriptPath As String\r\n"
    "    scriptPath = base & \"src\" & Application.PathSeparator & \"generate_pdf.py\"\r\n"
    "\r\n"
    "    If Dir(scriptPath) = \"\" Then\r\n"
    "        FileCopy formPdf, finalPdf\r\n"
    "        GoTo ShowResult\r\n"
    "    End If\r\n"
    "\r\n"
    "    Dim cmd As String\r\n"
    "    cmd = \"cmd /c python \" & q & scriptPath & q & _\r\n"
    "          \" --excel-pdf \" & q & formPdf & q & _\r\n"
    "          \" --invoices \" & q & base & \"invoices\" & q & _\r\n"
    "          \" --output \" & q & finalPdf & q & _\r\n"
    "          \" > \" & q & logPath & q & \" 2>&1\"\r\n"
    "    Shell cmd, vbHide\r\n"
    "\r\n"
    "    Application.StatusBar = \"Generation du PDF en cours...\"\r\n"
    "    Dim t As Date : t = Now()\r\n"
    "    Do\r\n"
    "        DoEvents\r\n"
    "        Application.Wait Now + TimeValue(\"00:00:01\")\r\n"
    "        If Dir(finalPdf) <> \"\" Then Exit Do\r\n"
    "        If DateDiff(\"s\", t, Now()) > 90 Then Exit Do\r\n"
    "    Loop\r\n"
    "    Application.StatusBar = False\r\n"
    "\r\n"
    "ShowResult:\r\n"
    "    If Dir(finalPdf) <> \"\" Then\r\n"
    "        Dim rep As Integer\r\n"
    "        rep = MsgBox(\"PDF genere avec succes !\" & vbCrLf & _\r\n"
    "                     finalPdf & vbCrLf & vbCrLf & _\r\n"
    "                     \"Reinitialiser le formulaire pour le prochain enfant ?\" & vbCrLf & _\r\n"
    "                     \"(Efface enfant, factures saisies et images)\", _\r\n"
    "                     vbYesNo + vbQuestion, \"PDF cree\")\r\n"
    "        If rep = vbYes Then Call ResetPourProchainEnfant\r\n"
    "        Shell \"explorer.exe \" & q & finalPdf & q, vbNormalFocus\r\n"
    "    Else\r\n"
    "        Dim logTxt As String, fNum As Integer\r\n"
    "        fNum = FreeFile()\r\n"
    "        If Dir(logPath) <> \"\" Then\r\n"
    ...
    "                Line Input #fNum, ln\r\n"
    "                logTxt = logTxt & ln & vbCrLf\r\n"
    "            Loop\r\n"
    "            Close #fNum\r\n"
    "        End If\r\n"
    "        MsgBox \"Erreur : le PDF n'a pas ete genere.\" & vbCrLf & vbCrLf & _\r\n"
    "               IIf(logTxt <> \"\", Left(logTxt, 600), \"Verifiez que Python est installe.\"), _\r\n"
    "               vbCritical, \"Erreur PDF\"\r\n"
    "    End If\r\n"
    "End Sub\r\n"
    "\r\n"
    "' ----------------------------------------------------------------\r\n"
    "'  Reinitialiser pour le prochain enfant\r\n"
    "' ----------------------------------------------------------------\r\n"
